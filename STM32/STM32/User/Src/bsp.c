...
void Process_Cable_Cmd(MotorMsg *data) {
	RE35_Motor_SetCmd(board.cable_motor, data->motor_mode, data->vel, Cable_Convert_Pos(data->pos)); //  接收的速度：RPM，接收的位置：m
	RE35_Motor_Send(board.cable_motor);
}

void Process_Archor_Cmd(MotorMsg *data) {
	RE35_Motor_SetCmd(board.archor_motor, data->motor_mode, data->vel, Archor_Convert_Pos(data->pos)); //  接收的速度：RPM，接收的位置：m
...
void Process_A1_Cmd(MotorMsg *data) {
	A1_Motor_SetCmd(board.roll_motor, data->motor_mode, 0, A1_Convert_Vel(data->vel), data->pos); //  接收的速度：m/s，接收的位置：rad
	...
	if (false == data->start || (data->cdpr_traj == EMPTY1 && data->chassis_traj == EMPTY2)) {
		...
		if (data->chassis_traj == EMPTY2) {
			...
			switch (data->cdpr_traj) {
				case UPDOWN:
					local_traj = Read_CSV("updown.csv");
					break;
				case LINE:
					local_traj = Read_CSV("line.csv");
					break;
				case CIRCLE:
					local_traj = Read_CSV("circle.csv");
					break;
				default:
					break;
			}
			Start_Local_Traj(local_traj, data->period);
			Free_CSVData(local_traj);
		} else if (data->cdpr_traj == EMPTY1) {
			CSVData global_traj;
			switch (data->chassis_traj) {
				case NO_OBS:
					global_traj = Read_CSV("no_obs.csv");
					break;
				case OBS:
					global_traj = Read_CSV("obs.csv");
					break;
				default:
					break;
			}
			Start_Global_Traj(global_traj, data->period);
			Free_CSVData(global_traj);
		}
	}
}
