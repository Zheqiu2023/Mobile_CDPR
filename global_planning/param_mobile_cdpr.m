...
%% cdpr的初始位置    (世界坐标系下)
param_cdpr.robot_coor = [0; 0; 0; 1];
%% 4根绳索的起始固定位置 单位m  （车身坐标系下）
bp_x = [0.557;0.557;-0.557;-0.557];
bp_y = [0.545;-0.545;0.545;-0.545];
bp_z_min = [0.577;0.577;0.577;0.577];
bp_z_max = [1.09;1.09;1.09;1.09];

bp_z_init = bp_z_min + 0.15;
cl_init = [1.3394;1.3394;1.3394;1.3394];
...
param_cdpr.ep_o = ep_o; % 末端平台在自身坐标系下的位置
param_cdpr.ep_o_g = ep_o_g; % 末端平台在车身坐标系下的位置
%% 末端平台质量
M_ee = 1.0;   % kg
g = 9.81;
...
param_cdpr.g = g;
%% 绞盘
param_cdpr.dia_winch = 30*1e-3*ones(4,1); % 绞盘直径 单位m
%% 滑轮
...
%% 轮组
param_cdpr.wheel_radius = 0.1175;
%% 轨迹步长
