...
function [real_pose, real_cf] = direct_kine(h, l, plan_result_last, param_cdpr)
%% 求解正运动学
...
% boundary
lb=[-1.5;-1.5;-1.5;  -1;-1;-1;  0;0;0;0]; % 下限
ub=[1.5;1.5;1.5;  1;1;1;    inf;inf;inf;inf]; % 上限

% options
options = optimset('Display','final','TolFun',1e-21,'TolX',1e-21,'MaxIter',5000,'MaxFunEvals',5000*18);

% 更新锚点座坐标
[X, Y, Z] = fit_archor_coor(h);
param_cdpr.bp_coor(1, :) = X;
param_cdpr.bp_coor(2, :) = Y;
param_cdpr.bp_coor(3, :) = Z;

[result,resnorm,residual,exitflag,output] = lsqnonlin(@(x) direct_kine_lsqnonlin(x,l,param_cdpr),x0,lb,ub,options);
