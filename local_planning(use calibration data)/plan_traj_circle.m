...
for i = 1:t_num
    ...
    plan_result(:,i) = [real_h;real_cf;real_cl;real_pose]; 
    plan_result_last = [real_h;real_cf;real_cl;real_pose];    % 用来保证轨迹内相邻点的求解结果不跳变

    % 正运动学求解的期望位姿，写入文件中，与实际位姿对比
    fprintf(fid1, '%.5f,%.5f,%.5f,%.5f,%.5f,%.5f\n', real_pose);% 一行6个数据，用逗号分隔，每行结束后加上\n换行
    % 将结果写入.csv文件，用来控制电机运行
    fprintf(fid2, '%.5f,', real_h-param_cdpr.init_h);
