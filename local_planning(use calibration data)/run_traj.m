...
pose_plot = [];t_plot = [];h_plot = [];
plan_result_last = [param_cdpr.init_h;zeros(4,1);param_cdpr.cl_init;param_cdpr.ep_o_g(1:3);zeros(3,1)]; % 保存上一时刻的规划结果
...
for i = 1:length(param_traj)
    ...
    t_plot = [t_plot,t_vec]; h_plot = [h_plot,plan_result(1:4,:)];
end
% plot_robot_motion(t_plot,pose_plot,h_plot,param_cdpr);
