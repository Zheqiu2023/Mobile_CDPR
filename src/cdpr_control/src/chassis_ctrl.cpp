 ...
 * @param  cmd
 */
void ChassisCtrl::chassisCmdCB(const cdpr_bringup::ChassisCmd::ConstPtr& cmd)
{
    chassis_cmd_ = *cmd;
}
...
ChassisCtrl::ChassisCtrl(ros::NodeHandle& nh) : nh_(nh)
{
    ...
    cmd_chassis_sub_ =
        nh_.subscribe<cdpr_bringup::ChassisCmd>("/cmd_chassis", 10, boost::bind(&ChassisCtrl::chassisCmdCB, this, _1));
...
void ChassisCtrl::moveJoint()
{
    ...
    for (size_t i = 0; i < wheelsets_.size(); ++i)
    {
        ...
        double vel_angle = std::atan2(vel.y(), vel.x()) + wheelsets_[i].steer_offset_;
...
int main(int argc, char** argv)
{
    ...
    int update_rate = nh.param("update_rate", 100);
