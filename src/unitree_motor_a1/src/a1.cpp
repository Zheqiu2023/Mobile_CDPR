 ...
 * @param  port
 */
void A1Control::init(SerialPort& port)
{
    // 电机初始化参数
    init_param_.id = id_;
    init_param_.mode = 0;
    ...
    ROS_INFO("Initialization completed, start running!");
    ROS_INFO("Zero position: %f", motor_zero_position_);
}

/**
 * @brief  根据控制模式设置参数K_W、K_P并运行电机
 */
void A1Control::drive(SerialPort& port, const int& ctrl_frequency)
{
    ...
    if (cmd_type == 0)
    {
        ...
        for (auto goal_position : goal_position_vec)
        {
            ...
            ROS_DEBUG_STREAM("Receivedposition: " << motor_recv_.Pos);
        }
    }
    else if (cmd_type == 1)
    {
        ...
        // 设置目标速度
        ros::param::get("/a1/motor_ctrl_data/goal_vel", motor_cmd_.W);
        motor_cmd_.W *= reduction_ratio;

        while (ros::ok())
        {
            port.sendRecv(&motor_cmd_, &motor_recv_);
            ROS_DEBUG_STREAM("Received velocity: " << motor_recv_.W);
        }
    }
    else if (cmd_type == 2)
    {
        ...
        // 设置目标力矩
        ros::param::get("/a1/motor_ctrl_data/goal_trq", motor_cmd_.T);

        while (ros::ok())
        {
            port.sendRecv(&motor_cmd_, &motor_recv_);
            ROS_DEBUG_STREAM("Received torque: " << motor_recv_.T);
        }
    }
    else if (cmd_type == 3)
    {
        ...
        else if ((right_now - start_time_) >= total_run_time)  // 表示整条路径已经走完
        {
            ...
            ROS_INFO("Motor A1 run complete!");
        }
    ...
    else if (cmd_type == 4)
    {
        ...
        {
            ROS_INFO("Motor A1 %d run complete!", motor_cmd_.id);
 ...
 * @param  port
 */
void A1Control::stall(SerialPort& port)
{
...
void A1Run::operator()()
{
    ...
    ros::param::get("/a1/motor_ctrl_data/ctrl_frequency", ctrl_frequency_);

    ros::Rate loop_rate(ctrl_frequency_);
    while (ros::ok())
    {
        a1_control_.drive(serial, ctrl_frequency_);
        loop_rate.sleep();
    }
