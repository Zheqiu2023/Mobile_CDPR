...
load('vertexPosition_P'); % 3X8, P-coordinate vertex positions
%% 定义点的拓扑信息(手动添加邻点)
for i=1:8
    vertex(i).coordinate = vertexPosition_P(:,i); %点坐标
        if i>=1 && i<=4  %定义邻接点
            if i == 1 
                vertex(i).n_verticesId = [2 4 5];
            elseif i == 4
                vertex(i).n_verticesId = [1 3 8];
            else
                vertex(i).n_verticesId =  [i-1 i+1 i+4];
            end
        else 
            if i==5
                vertex(i).n_verticesId = [1 6 8];
            elseif i==8
                vertex(i).n_verticesId = [4 5 7];
            else
                vertex(i).n_verticesId =  [i-4 i-1 i+1];
            end
        end
end
%% 定义边的拓扑信息
...
%按点相邻关系创建面
for i=1:8
    vecOi = vertex(i).coordinate; %原点到点i的向量（用于判断外法向量方向）
    for j=1:size(vertex(i).n_verticesId, 2) %遍历点i的邻接点
        temp_nVer1 = vertex(i).n_verticesId(j); %邻接点1
        if j == size(vertex(i).n_verticesId, 2)
            temp_nVer2 = vertex(i).n_verticesId(1); %邻接点2
        else
            temp_nVer2 = vertex(i).n_verticesId(j+1); %邻接点2
        end
        tempNew = [i temp_nVer1 temp_nVer2];  %待审查面邻接关系的点
        tempVec1 = vertex(temp_nVer1).coordinate - vecOi; %temp vector1 (Oi to vertex1)
        tempVec2 = vertex(temp_nVer2).coordinate - vecOi; %temp vector2 (Oi to vertex2)
        temp_normal = cross(tempVec1, tempVec2); %temp normal vector of the face
        temp_normal = temp_normal/norm(temp_normal); %归一化
        if dot(temp_normal, vecOi)<0  %判断法向量是否朝外
...
%末端平台包络为长方体，且相邻点已按从小到大顺序排列好了（逆时针或顺时针），因此需再通过判断相邻两边叉乘方向判断是否逆时针排列
for i=1:8
    vecOi = vertex(i).coordinate; %原点到点Oi的向量（用于判断外法向量方向）
    Ver1 = vertex(i).n_verticesId(1); %temp neighbour vertex1
    Ver2 = vertex(i).n_verticesId(2); %temp neighbour vertex2
    Vec1 = vertex(Ver1).coordinate - vecOi; %temp vector1 (Oi to vertex1)
    Vec2 = vertex(Ver2).coordinate - vecOi; %temp vector2 (Oi to vertex2)
    tempCross = cross(Vec1, Vec2);
    if dot(tempCross, vecOi)<0  %顺时针排列
